...
module.exports = client => {
  ...
  router.get("/add", checkAuth, async (req, res) => {
    ...
    if (inguild === false) {
      error = {
        html: `<p class="mb-0">Voce deve entrar na Dark Web on para adicionar um bot: <br>  <a id="invite" href="${client.config.invite}" class="btn btn-sm btn-outline-danger">${client.config.invite}</a></p>`,
        type: "danger"
      };
    }
  ...
  router.get("/:id/edit", checkAuth, async (req, res) => {
    ...
    if (inguild === false) {
      error = {
        html: `<p class="mb-0">Você precisa se juntar ao nosso servidor para enviar um bot. <br>  <a id="invite" href="${client.config.invite}" class="btn btn-sm btn-outline-danger">${client.config.invite}</a></p>`,
        type: "danger"
      };
    }
    let id = req.params.id;
  ...
  router.post("/:id/edit", checkAuth, async (req, res) => {
    ...
    await bot2.save().then(async () => {
      res.status(201).json({ message: "editou o bot", code: "OK" });
      let logs = guild.channels.cache.get(client.config.channels.logs);
      logs.send(
        `<@${user.id}> editou **${bot.username}#${bot.discriminator}**`
      );
    });
  });
  router.post("/add/submit", checkAuth, async (req, res) => {
    ...
    try {
      ...
      return res.status(409).json({ message: "Este não é um bot de verdade" });
    }

    if (!bot.bot) return res.status(409).json({ message: "Isto não é um bot" });
    if (data.onwers) {
      data.owners.forEach(async owner => {
        try {
          ...
          return res
            .status(409)
            .json({ message: "Um de seus proprietários não é um usuário" });
        }
      });
    }
    let inguild = guild.members.cache.get(req.user.id) ? true : false;
    if (inguild === false) {
      return res
        .status(409)
        .json({ message: "Você deve se juntar ao nosso Discord Server para enviar um bot" });
    } else if (await model.findOne({ bot: data.id })) {
      return res
        .status(409)
        .json({ message: "O bot já foi enviado" });
    } else {
      await model
        .create({
          bot: data.id,
        ...
        .then(async () => {
          res.status(201).json({ message: "Adicionado a Fila", code: "OK" });
          let logs = guild.channels.cache.get(client.config.channels.logs);
          logs.send(
            `O usuario: <@${user.id}> adicionou o bot: **${bot.username}#${bot.discriminator}** para ser verificado!`
          );
        });
    }
  });
  ...
  router.post("/:id/delete", checkAuth, async (req, res) => {
    ...
    await model.findOneAndDelete({ bot: id }).then(async () => {
      let logs = guild.channels.cache.get(client.config.channels.logs);
      logs.send(
        `O bot: **${bot.username}#${bot.discriminator}** foi deletado por: <@${user.id}>`
      );
      client.guilds.cache
        .get(client.config.guilds.main)
        .members.cache.get(bot.id)
        .kick({ reason: "Um bot mau" });
      res.redirect("/");
    });
  });
  router.post("/:id/vote", checkAuth, async (req, res) => {
    ...
    let logs = guild.channels.cache.get(client.config.channels.logs);
    logs.send(
      `O usuario: <@${req.user.id}> votou no bot: **${BotRaw.username}#${BotRaw.discriminator}**`
    );
